...
Check the labels marked with red rectangle. These labels verifies that the metrics are coming from `MongoDB` database `builtin-prom-mgo` through stats service `builtin-prom-mgo-stats`.
