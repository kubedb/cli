...
The installer script and associated yaml files can be found in the [/hack/deploy](https://github.com/kubedb/cli/tree/0.11.0/hack/deploy) folder. You can see the full list of flags available to installer using `-h` flag.

```console
$ curl -fsSL https://raw.githubusercontent.com/kubedb/cli/0.11.0/hack/deploy/kubedb.sh | bash -s -- -h
...
KubeDB 0.11.0 or later installs a catalog of database versions. To disable this pass the `--install-catalog=none` flag.
...
appscode/kubedb        	0.11.0 	0.11.0	KubeDB by AppsCode - Production ready databases on Kubern...
appscode/kubedb-catalog	0.11.0 	0.11.0	KubeDB Catalog by AppsCode - Catalog for database versions
...
$ helm upgrade kubedb-catalog appscode/kubedb-catalog --version 0.11.0 \
  --namespace kube-system
```
...
# Mac 64-bit
wget -O kubedb https://github.com/kubedb/cli/releases/download/0.11.0/kubedb-darwin-amd64 \
  && chmod +x kubedb \
  && sudo mv kubedb /usr/local/bin/
```
