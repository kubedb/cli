...
Now, open your browser and go to the following URL: _http://{minikube-ip}:{pgadmin-svc-nodeport}_. According to the above example, this URL will be [http://192.168.99.100:31188](http://192.168.99.100:31188). To log into the phpMyAdmin, use username __`admin`__ and password __`admin`__.
...
Here,
 - `spec.version` is the version of MySQL database. In this tutorial, a MySQL 9.5 database is going to be created.
 ...
 - `spec.init.scriptSource` specifies a bash script used to initialize the database after it is created. In this tutorial, `run.sh` script from the git repository `https://github.com/kubedb/mysql-init-scripts.git` is used to create a `dashboard` table in `data` schema.
...
Events:
  ...
  ---------   --------   -----     ----                --------   ------               -------
  2m          2m         1         MySQL operator   Normal     SuccessfulValidate   Successfully validate MySQL
  2m          2m         1         MySQL operator   Normal     SuccessfulCreate     Successfully created MySQL
  2m          2m         1         MySQL operator   Normal     SuccessfulCreate     Successfully created StatefulSet
...
```yaml
$ kubedb get pg -n demo m1 -o yaml
apiVersion: kubedb.com/v1alpha1
kind: MySQL
metadata:
  ...
  selfLink: /apis/kubedb.com/v1alpha1/namespaces/demo/mysqles/p1
...
Please note that KubeDB operator has created a new Secret called `p1-admin-auth` (format: {tpr-name}-admin-auth) for storing the password for `mysql` superuser. This secret contains a `.admin` key with a ini formatted key-value pairs. If you want to use an existing secret please specify that when creating the tpr using `spec.databaseSecret.secretName`.

Now, you can connect to this database from the phpMyAdmin dashboard using the database pod IP and `mysql` user password. Now, open your browser and go to the following URL: _http://{minikube-ip}:{pgadmin-svc-nodeport}_. To log into the phpMyAdmin, use username __`admin`__ and password __`admin`__.
...
![Using m1 from phpMyAdmin4](/docs/images/mysql/p1-pgadmin.gif)
...
Snapshots:
  ...
  ----     ------      ---------                         --------------                    -----
  m1-xyz   gs:restic   Mon, 17 Jul 2017 19:18:00 -0700   Mon, 17 Jul 2017 19:19:11 -0700   Succeeded
...
Events:
  ...
  33m         33m        1         MySQL operator     Normal     SuccessfulValidate   Successfully validate MySQL
  33m         33m        1         MySQL operator     Normal     SuccessfulCreate     Successfully created StatefulSet
  33m         33m        1         MySQL operator     Normal     SuccessfulCreate     Successfully created MySQL
  34m         34m        1         MySQL operator     Normal     Creating             Creating Kubernetes objects
  34m         34m        1         MySQL operator     Normal     SuccessfulValidate   Successfully validate MySQL
```

Once the snapshot Job is complete, you should see the output of the `pg_dump` command stored in the GCS bucket.

![snapshot-console](/docs/images/mysql/p1-xyz-snapshot.png)
...
$ kubectl create -f ./docs/examples/mysql/demo-4.yaml
...
Events:
  ...
  ---------   --------   -----     ----                --------   ------               -------
  3m          3m         1         MySQL operator   Normal     SuccessfulValidate   Successfully validate MySQL
  3m          3m         1         MySQL operator   Warning    Failed               Failed to complete initialization
  3m          3m         1         MySQL operator   Normal     SuccessfulCreate     Successfully created MySQL
  5m          5m         1         MySQL operator   Normal     SuccessfulCreate     Successfully created StatefulSet
  5m          5m         1         MySQL operator   Normal     Initializing         Initializing from Snapshot: "p1-xyz"
  5m          5m         1         MySQL operator   Normal     Creating             Creating Kubernetes objects
  5m          5m         1         MySQL operator   Normal     SuccessfulValidate   Successfully validate MySQL
```
...
```console
$ kubedb delete pg m1 -n demo
error: MySQL "p1" can't be paused. To continue delete, unset spec.doNotPause and retry.
```

Now, run `kubedb edit pg m1 -n demo` to set `spec.doNotPause` to false or remove this field (which default to false). Then if you delete the MySQL tpr, KubeDB operator will delete the StatefulSet and its pods, but leaves the PVCs unchanged. In KubeDB parlance, we say that `p1` MySQL database has entered into dormant state. This is represented by KubeDB operator by creating a matching DormantDatabase tpr.

```yaml
$ kubedb delete pg -n demo m1
mysql "p1" deleted
...
You still have a record that there used to be a MySQL database `p1` in the form of a DormantDatabase database `p1`. Since you have already wiped out the database, you can delete the DormantDatabase tpr. 
