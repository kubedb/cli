...
Note that, Kubedb does not allow `MYSQL_ROOT_PASSWORD` environment variable to set in `spec.env`. If you want to set the root password, please use `spec.databaseSecret` instead described earlier.

If you try to set `MYSQL_ROOT_PASSWORD` environment variable in MySQL crd, Kubed operator will reject the request with following error,
```
