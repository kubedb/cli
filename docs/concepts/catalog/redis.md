...
`spec.podSecurityPolicies.databasePolicyName` is a required field that specifies the name of the pod security policy required to get the database server pod(s) running. To use a user-defined policy, name of the policy have to be added here and in the list of allowed policy names during kubedb operator installation like this: 
```bash
