...
### spec.version
`spec.version` is a required field specifying the version of MySQL database. Currently the supported value is `8.0`.
...
To initialize a MySQL database using a script (shell script, db migrator, etc.), set the `spec.init.scriptSource` section when creating a MySQL object. ScriptSource must have following information:
...
In the above example, KubeDB operator will launch a Job to execute all sql script of `mysql-init-script` repo in alphabetical  order script once StatefulSet pods are running.
