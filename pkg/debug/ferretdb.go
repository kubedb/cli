...
func FerretDBDebugCMD(f cmdutil.Factory) *cobra.Command {
	var (
		dbName            string
	...
	mdDebugCmd := &cobra.Command{
		...
		Short:   "Debug helper for FerretDB database",
		Example: `kubectl dba debug ferretdb -n demo sample-ferretdb --operator-namespace kubedb`,
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) == 0 {
				log.Fatal("Enter ferretdb object's name as an argument")
			}
			dbName = args[0]
			...
			gvk := func() schema.GroupVersionKind {
				kind := olddbapi.ResourceKindFerretDB
