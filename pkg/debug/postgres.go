...
func PostgresDebugCMD(f cmdutil.Factory) *cobra.Command {
	var (
		dbName            string
		operatorNamespace string
	)

	pgDebugCmd := &cobra.Command{
		Use: "postgres",
		Aliases: []string{
			"pg",
		},
	...
	pgDebugCmd.Flags().StringVarP(&operatorNamespace, "operator-namespace", "o", "kubedb", "the namespace where the kubedb operator is installed")
...
func (opts *postgresOpts) collectOperatorLogs() error {
...
func (opts *postgresOpts) collectForAllDBPetSets() error {
...
func (opts *postgresOpts) writeLogs(podName, ns, container string) error {
...
func (opts *postgresOpts) collectOtherYamls() error {
	opsReqs, err := opts.dbClient.OpsV1alpha1().PostgresOpsRequests(opts.db.Namespace).List(context.TODO(), metav1.ListOptions{})
	if err != nil {
		return err
	}
	...
	scalers, err := opts.dbClient.AutoscalingV1alpha1().PostgresAutoscalers(opts.db.Namespace).List(context.TODO(), metav1.ListOptions{})
	if err != nil {
		return err
	}
