...
func OracleDebugCMD(f cmdutil.Factory) *cobra.Command {
	var (
		dbName            string
	...
	mdDebugCmd := &cobra.Command{
		...
		Short:   "Debug helper for Oracle database",
		Example: `kubectl dba debug oracle -n demo sample-oracle --operator-namespace kubedb`,
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) == 0 {
				log.Fatal("Enter oracle object's name as an argument")
			}
			dbName = args[0]
			...
			gvk := func() schema.GroupVersionKind {
				kind := olddbapi.ResourceKindOracle
