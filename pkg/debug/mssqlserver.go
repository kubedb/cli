...
func MSSQLServerDebugCMD(f cmdutil.Factory) *cobra.Command {
	var (
		dbName            string
	...
	mdDebugCmd := &cobra.Command{
		...
		Short:   "Debug helper for MSSQLServer database",
		Example: `kubectl dba debug mssqlserver -n demo sample-mssqlserver --operator-namespace kubedb`,
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) == 0 {
				log.Fatal("Enter mssqlserver object's name as an argument")
			}
			dbName = args[0]
			...
			gvk := func() schema.GroupVersionKind {
				kind := olddbapi.ResourceKindMSSQLServer
