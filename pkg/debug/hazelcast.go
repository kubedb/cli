...
func HazelcastDebugCMD(f cmdutil.Factory) *cobra.Command {
	var (
		dbName            string
	...
	mdDebugCmd := &cobra.Command{
		...
		Short:   "Debug helper for Hazelcast database",
		Example: `kubectl dba debug hazelcast -n demo sample-hazelcast --operator-namespace kubedb`,
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) == 0 {
				log.Fatal("Enter hazelcast object's name as an argument")
			}
			dbName = args[0]
