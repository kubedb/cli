...
func ZooKeeperDebugCMD(f cmdutil.Factory) *cobra.Command {
	var (
		dbName            string
	...
	mdDebugCmd := &cobra.Command{
		...
		Short:   "Debug helper for Zookeeper database",
		Example: `kubectl dba debug zookeeper -n demo sample-zookeeper --operator-namespace kubedb`,
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) == 0 {
				log.Fatal("Enter zookeeper object's name as an argument")
			}
			dbName = args[0]
			...
			gvk := func() schema.GroupVersionKind {
				kind := olddbapi.ResourceKindZooKeeper
