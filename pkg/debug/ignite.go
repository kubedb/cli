...
func IgniteDebugCMD(f cmdutil.Factory) *cobra.Command {
	var (
		dbName            string
	...
	mdDebugCmd := &cobra.Command{
		...
		Short:   "Debug helper for Ignite database",
		Example: `kubectl dba debug ignite -n demo sample-ignite --operator-namespace kubedb`,
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) == 0 {
				log.Fatal("Enter ignite object's name as an argument")
			}
			dbName = args[0]
			...
			gvk := func() schema.GroupVersionKind {
				kind := olddbapi.ResourceKindIgnite
