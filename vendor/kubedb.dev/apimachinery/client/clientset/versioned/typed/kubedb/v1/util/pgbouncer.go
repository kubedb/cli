...
func CreateOrPatchPgBouncer(ctx context.Context, c cs.KubedbV1Interface, meta metav1.ObjectMeta, transform func(*api.PgBouncer) *api.PgBouncer, opts metav1.PatchOptions) (*api.PgBouncer, kutil.VerbType, error) {
	cur, err := c.PgBouncers(meta.Namespace).Get(ctx, meta.Name, metav1.GetOptions{})
	if kerr.IsNotFound(err) {
		klog.V(3).Infof("Creating PgBouncer %s/%s.", meta.Namespace, meta.Name)
...
func PatchPgBouncer(ctx context.Context, c cs.KubedbV1Interface, cur *api.PgBouncer, transform func(*api.PgBouncer) *api.PgBouncer, opts metav1.PatchOptions) (*api.PgBouncer, kutil.VerbType, error) {
	return PatchPgBouncerObject(ctx, c, cur, transform(cur.DeepCopy()), opts)
}

func PatchPgBouncerObject(ctx context.Context, c cs.KubedbV1Interface, cur, mod *api.PgBouncer, opts metav1.PatchOptions) (*api.PgBouncer, kutil.VerbType, error) {
	...
	klog.V(3).Infof("Patching PgBouncer %s/%s with %s.", cur.Namespace, cur.Name, string(patch))
	out, err := c.PgBouncers(cur.Namespace).Patch(ctx, cur.Name, types.MergePatchType, patch, opts)
	return out, kutil.VerbPatched, err
}

func TryUpdatePgBouncer(ctx context.Context, c cs.KubedbV1Interface, meta metav1.ObjectMeta, transform func(*api.PgBouncer) *api.PgBouncer, opts metav1.UpdateOptions) (result *api.PgBouncer, err error) {
	attempt := 0
