...
func CreateOrPatchMemcached(ctx context.Context, c cs.KubedbV1Interface, meta metav1.ObjectMeta, transform func(*api.Memcached) *api.Memcached, opts metav1.PatchOptions) (*api.Memcached, kutil.VerbType, error) {
	cur, err := c.Memcacheds(meta.Namespace).Get(ctx, meta.Name, metav1.GetOptions{})
	if kerr.IsNotFound(err) {
		klog.V(3).Infof("Creating Memcached %s/%s.", meta.Namespace, meta.Name)
...
func PatchMemcached(ctx context.Context, c cs.KubedbV1Interface, cur *api.Memcached, transform func(*api.Memcached) *api.Memcached, opts metav1.PatchOptions) (*api.Memcached, kutil.VerbType, error) {
	return PatchMemcachedObject(ctx, c, cur, transform(cur.DeepCopy()), opts)
}

func PatchMemcachedObject(ctx context.Context, c cs.KubedbV1Interface, cur, mod *api.Memcached, opts metav1.PatchOptions) (*api.Memcached, kutil.VerbType, error) {
	...
	klog.V(3).Infof("Patching Memcached %s/%s with %s.", cur.Namespace, cur.Name, string(patch))
	out, err := c.Memcacheds(cur.Namespace).Patch(ctx, cur.Name, types.MergePatchType, patch, opts)
	return out, kutil.VerbPatched, err
}

func TryUpdateMemcached(ctx context.Context, c cs.KubedbV1Interface, meta metav1.ObjectMeta, transform func(*api.Memcached) *api.Memcached, opts metav1.UpdateOptions) (result *api.Memcached, err error) {
	attempt := 0
	err = wait.PollUntilContextTimeout(ctx, kutil.RetryInterval, kutil.RetryTimeout, true, func(ctx context.Context) (bool, error) {
		...
		klog.Errorf("Attempt %d failed to update Memcached %s/%s due to %v.", attempt, cur.Namespace, cur.Name, e2)
		return false, nil
	})
...
func UpdateMemcachedStatus(
	...
	if err != nil {
		err = fmt.Errorf("failed to update status of Memcached %s/%s after %d attempts due to %v", meta.Namespace, meta.Name, attempt, err)
	}
	return
}
