...
func CreateOrPatchMariaDB(ctx context.Context, c cs.KubedbV1Interface, meta metav1.ObjectMeta, transform func(*api.MariaDB) *api.MariaDB, opts metav1.PatchOptions) (*api.MariaDB, kutil.VerbType, error) {
	cur, err := c.MariaDBs(meta.Namespace).Get(ctx, meta.Name, metav1.GetOptions{})
	if kerr.IsNotFound(err) {
		klog.V(3).Infof("Creating MariaDB %s/%s.", meta.Namespace, meta.Name)
...
func PatchMariaDB(ctx context.Context, c cs.KubedbV1Interface, cur *api.MariaDB, transform func(*api.MariaDB) *api.MariaDB, opts metav1.PatchOptions) (*api.MariaDB, kutil.VerbType, error) {
	return PatchMariaDBObject(ctx, c, cur, transform(cur.DeepCopy()), opts)
}

func PatchMariaDBObject(ctx context.Context, c cs.KubedbV1Interface, cur, mod *api.MariaDB, opts metav1.PatchOptions) (*api.MariaDB, kutil.VerbType, error) {
	...
	klog.V(3).Infof("Patching MariaDB %s/%s with %s.", cur.Namespace, cur.Name, string(patch))
	out, err := c.MariaDBs(cur.Namespace).Patch(ctx, cur.Name, types.MergePatchType, patch, opts)
	return out, kutil.VerbPatched, err
}

func TryUpdateMariaDB(ctx context.Context, c cs.KubedbV1Interface, meta metav1.ObjectMeta, transform func(*api.MariaDB) *api.MariaDB, opts metav1.UpdateOptions) (result *api.MariaDB, err error) {
	attempt := 0
	err = wait.PollUntilContextTimeout(ctx, kutil.RetryInterval, kutil.RetryTimeout, true, func(ctx context.Context) (bool, error) {
		...
		klog.Errorf("Attempt %d failed to update MariaDB %s/%s due to %v.", attempt, cur.Namespace, cur.Name, e2)
		return false, nil
	})
