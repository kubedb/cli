...
func CreateOrPatchProxySQL(ctx context.Context, c cs.KubedbV1Interface, meta metav1.ObjectMeta, transform func(*api.ProxySQL) *api.ProxySQL, opts metav1.PatchOptions) (*api.ProxySQL, kutil.VerbType, error) {
	cur, err := c.ProxySQLs(meta.Namespace).Get(ctx, meta.Name, metav1.GetOptions{})
	if kerr.IsNotFound(err) {
		klog.V(3).Infof("Creating ProxySQL %s/%s.", meta.Namespace, meta.Name)
...
func PatchProxySQL(ctx context.Context, c cs.KubedbV1Interface, cur *api.ProxySQL, transform func(*api.ProxySQL) *api.ProxySQL, opts metav1.PatchOptions) (*api.ProxySQL, kutil.VerbType, error) {
	return PatchProxySQLObject(ctx, c, cur, transform(cur.DeepCopy()), opts)
}

func PatchProxySQLObject(ctx context.Context, c cs.KubedbV1Interface, cur, mod *api.ProxySQL, opts metav1.PatchOptions) (*api.ProxySQL, kutil.VerbType, error) {
	...
	klog.V(3).Infof("Patching ProxySQL %s/%s with %s.", cur.Namespace, cur.Name, string(patch))
	out, err := c.ProxySQLs(cur.Namespace).Patch(ctx, cur.Name, types.MergePatchType, patch, opts)
	return out, kutil.VerbPatched, err
}

func TryUpdateProxySQL(ctx context.Context, c cs.KubedbV1Interface, meta metav1.ObjectMeta, transform func(*api.ProxySQL) *api.ProxySQL, opts metav1.UpdateOptions) (result *api.ProxySQL, err error) {
	attempt := 0
	err = wait.PollUntilContextTimeout(ctx, kutil.RetryInterval, kutil.RetryTimeout, true, func(ctx context.Context) (bool, error) {
		...
		klog.Errorf("Attempt %d failed to update ProxySQL %s/%s due to %v.", attempt, cur.Namespace, cur.Name, e2)
		return false, nil
	})
