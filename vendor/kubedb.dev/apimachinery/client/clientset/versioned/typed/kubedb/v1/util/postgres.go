...
func CreateOrPatchPostgres(ctx context.Context, c cs.KubedbV1Interface, meta metav1.ObjectMeta, transform func(*api.Postgres) *api.Postgres, opts metav1.PatchOptions) (*api.Postgres, kutil.VerbType, error) {
	cur, err := c.Postgreses(meta.Namespace).Get(ctx, meta.Name, metav1.GetOptions{})
	if kerr.IsNotFound(err) {
		klog.V(3).Infof("Creating Postgres %s/%s.", meta.Namespace, meta.Name)
...
func PatchPostgres(ctx context.Context, c cs.KubedbV1Interface, cur *api.Postgres, transform func(*api.Postgres) *api.Postgres, opts metav1.PatchOptions) (*api.Postgres, kutil.VerbType, error) {
	return PatchPostgresObject(ctx, c, cur, transform(cur.DeepCopy()), opts)
}

func PatchPostgresObject(ctx context.Context, c cs.KubedbV1Interface, cur, mod *api.Postgres, opts metav1.PatchOptions) (*api.Postgres, kutil.VerbType, error) {
	...
	klog.V(3).Infof("Patching Postgres %s/%s with %s.", cur.Namespace, cur.Name, string(patch))
	out, err := c.Postgreses(cur.Namespace).Patch(ctx, cur.Name, types.MergePatchType, patch, opts)
	return out, kutil.VerbPatched, err
}

func TryUpdatePostgres(ctx context.Context, c cs.KubedbV1Interface, meta metav1.ObjectMeta, transform func(*api.Postgres) *api.Postgres, opts metav1.UpdateOptions) (result *api.Postgres, err error) {
	attempt := 0
	err = wait.PollUntilContextTimeout(ctx, kutil.RetryInterval, kutil.RetryTimeout, true, func(ctx context.Context) (bool, error) {
		...
		klog.Errorf("Attempt %d failed to update Postgres %s/%s due to %v.", attempt, cur.Namespace, cur.Name, e2)
		return false, nil
	})
