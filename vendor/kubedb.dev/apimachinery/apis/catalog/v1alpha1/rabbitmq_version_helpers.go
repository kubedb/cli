...
func (RabbitMQVersion) CustomResourceDefinition() *apiextensions.CustomResourceDefinition {
...
func (r RabbitMQVersion) ValidateSpecs() error {
	if r.Spec.Version == "" ||
		...
		return fmt.Errorf("atleast one of the following specs is not set for RabbitMQVersion %q: %s", r.Name, strings.Join(fields, ", "))
	}
	return nil
}
